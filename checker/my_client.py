...
class RpcBackdoorPacket(RpcPacket):
    def __init__(self, route_key):
        super().__init__()
        self.packet_type = p32(6)
        self.route_key = route_key.encode('UTF-8')
        self.length += 8 + len(self.route_key)
...
class RpcClient(object):
    ...
    def try_backdoor(self, key):
        return self.conn.send_expect(
            RpcBackdoorPacket(key).into(),
            RpcReplyUnavailablePacket().into()
        )
